...
- Frontend source files live in `public/`; the Vercel FastAPI app serves embedded copies of those assets.
...
Hosted upload note: Vercel Functions have a 4.5 MB request body limit. All PDFs are extracted in the browser first, and the app sends only extracted text to the API. Large non-PDF uploads should be converted to TXT or reduced below 4 MB.
...
- Hosted Vercel uploads are limited by the platform request body limit. PDFs are processed through browser-side text extraction, but scanned PDFs may still need OCR or a TXT export.
